...
«источник → камеры → участки», 4 перспективных ОКС (два кластеризуются),
ограничения всех четырёх типов. Значения подобраны так, чтобы сработали
все механики: кластеризация, врезка в камеру и в трубу, реконструкция,
спецпроход, предельная длина.
"""
...
def main():
    features = [
        ...
        # B4 — отдельно, западнее; большой расход → реконструкция S1
